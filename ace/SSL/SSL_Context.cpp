// $Id: SSL_Context.cpp 2628 2015-08-14 15:17:29Z mitza $
      ...
      if (ACE::debug ())
        {
          ACELIB_DEBUG ((LM_DEBUG,
                         ...
                         ACE_TEXT ("peerssl is null, or get host name failed\n")));
        }
      return false;
    }
  ...
  if (ACE::debug ())
    {
      ACELIB_DEBUG ((LM_DEBUG,
                     ACE_TEXT ("ACE (%P|%t) SSL_Context::check_host ")
                     ACE_TEXT ("not supported by openssl\n")));
    }
  ACE_UNUSED_ARG (host);
